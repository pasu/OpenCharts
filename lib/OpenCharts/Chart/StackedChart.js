 ...
 *  - <OpenCharts.Chart>
 */
OpenCharts.Chart.StackedChart = OpenCharts.Class(OpenCharts.Chart,{
        ...
        if(me.coordinateStyle.bShowAxes && me.itemArray != null)
        {
            ...
            if(me.coordinateStyle.bShowXAxesText)
            {
                if(me.coordinateStyle.axesTextStyle.bFillText)
                {
                    ...
                    {
                        var xBegin= (1+(i+1)+i)*me.nXRange + me.nXRange/2;
    ...
    calculateValue:function(){
        ...
        if(me.itemArray.length>1)
        {
            var tempMax = 0;
            for(var i=0;i<me.itemArray[0].length;i++)
            {
                tempMax = 0;
                for(var j=0;j<me.itemArray.length;j++)
                {
                    tempMax += me.itemArray[j][i].data;
                }
                if(tempMax>me.maxValue)
                    me.maxValue = tempMax;
            }
        }
        ...
        //所有项、空白、以及坐标轴两侧的空隙
        me.nXRange = me.width/(me.nMaxSpace+1 + me.nMaxSpace + 2);
    ...
    renderChartByAnimation:function(step){
        ...
        for(var i=0;i<me.itemArray.length;i++)
        {
            ...
            {
                var barStyle = me.itemArray[i][0].itemStyle;
                ...
                var xBegin= (1+(j+1)+j)*me.nXRange;

                var yBegin=me.height - me.coordinateStyle.footerRange;
                for(var k=0;k<i;k++)
                {
                    yBegin -=me.itemArray[k][j].data / stepYValue * nYRange;
                }
                var nHeight=me.itemArray[i][j].data / stepYValue * nYRange;

                me.myContext.fillRect(xBegin,yBegin,me.nXRange,-nHeight*step/10);
            }
        }
    ...
    getItemByPosition:function(pointX,pointY,selectType){
        ...
        for(var i=0;i<me.itemArray.length;i++)
        {
            for(var j=0;j<me.itemArray[i].length;j++)
            {
                ...
                var nHeight=me.itemArray[i][j].data / stepYValue * nYRange;
                var yEnd=yBegin-nHeight;
                if(pointX>=xBegin && pointX<xEnd
                    && pointY>=yEnd && pointY<yBegin)
                {
    ...
    CLASS_NAME: "OpenCharts.Chart.StackedChart"
});